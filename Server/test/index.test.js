...
describe("Test de RUTAS", () => {
    ...
    describe("GET /rickandmorty/login", () => {
        it('Las credenciales son correctas', async () => {
            const response = (await agent.get('/rickandmorty/login?email=isdavid92@hotmail.com&password=admin123')).body;
            expect(response.access).toBeTruthy()
        })

        it('Las credenciales son incorrectas', async () => {
            const response = (await agent.get('/rickandmorty/login?email=isra@hotmail.com&password=admonda123')).body;
